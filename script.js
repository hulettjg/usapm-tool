...
// MODIFIED: initializeDashboard now checks for share links on load
function initializeDashboard() {
    ...
    if (!checkForShareLink()) { // If no share link is found...
        checkForUpdates(null, false); // ...then check for version updates.
    }
    ...
    if (window.location.hash) {
        try {
            ...
            if (confirm('A shared deficiency has been detected. Do you want to load it into the form?')) {
                // Pre-fill the form without saving it to the user's list yet
                document.getElementById('currentId').value = deficiency.id || '';
            ...
            return true; // Indicate that a share link was processed
        } catch (e) {
...
async function checkForUpdates(event, isManualCheck = false) {
    if (event) event.preventDefault();
    if (isManualCheck) alert('Checking for the latest version...');
    try {
        const response = await fetch(VERSION_CHECK_URL + '?t=' + Date.now());
        if (!response.ok) throw new Error('Network response was not ok');
        const versionInfo = await response.json();
        if (isNewerVersion(versionInfo.latestVersion, CURRENT_VERSION)) {
            let updateMessage = `A new version (${versionInfo.latestVersion}) is available!\n\n`;
            updateMessage += `You are currently using version ${CURRENT_VERSION}.\n\n`;
            updateMessage += `What's New:\n${versionInfo.releaseNotes}\n\n`;
            updateMessage += `Click OK to refresh and load the new version.`;
            if (confirm(updateMessage)) {
                window.location.reload();
            }
        } else if (isManualCheck) {
            alert(`You are using the latest version (${CURRENT_VERSION}).`);
        }
    } catch (error) {
        console.error('Update check failed:', error);
        if (isManualCheck) alert('Could not check for updates. This might be due to network restrictions.');
    }
}
