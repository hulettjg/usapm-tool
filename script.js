...
function updateSlidePreview() {
    const finalCause = (document.getElementById('editCauseCode').value || "").replace(/^[A-Z0-9]+: /,'') || "[select cause code]";
    const correctiveAction = document.getElementById('editCapAction').value || "[insert process change]";
    const timeframe = document.getElementById('editCapTimeframe').value || "(insert timeframe)";
    ...
    document.getElementById('s_header').textContent = document.getElementById('editTracking').value || "TRACKING NUMBER";
    document.getElementById('s_body').textContent = finalCapStatement;
}
...
// MODIFIED: This function is now robust with a try/catch fallback.
